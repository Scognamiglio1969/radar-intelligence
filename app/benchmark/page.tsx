...
export default async function BenchmarkPage() {
  ...
  if (results.length === 0) {
    return (
      <>
        ...
        <EmptyState message="Nessuna entità da confrontare. Aggiungile da Gestione progetti (es. brand o competitor del settore)." />
      </>
    );
  }
  ...
  return (
    <>
      <PageHeader
        title="Benchmark"
        subtitle="Confronto di share of voice e sentiment sulle entità del settore (ultimi 14 giorni)"
      />
      ...
      <div className="grid gap-4 lg:grid-cols-3">
        <section className="panel px-5 py-4">
          ...
          {total > 0
            ...
            : <p className="py-16 text-center text-sm text-slate-500">Nessuna mention associata alle entità.</p>}
        </section>

        <section className="panel px-5 py-4 lg:col-span-2">
          <h2 className="mb-3 text-sm font-semibold text-slate-300">Trend del volume</h2>
      ...
      <section className="panel mt-4 overflow-x-auto px-5 py-4">
        <h2 className="mb-3 text-sm font-semibold text-slate-300">Dettaglio</h2>
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-[var(--border)] text-left text-xs uppercase tracking-wide text-slate-500">
              ...
              <th className="pb-2 text-right">Mention</th>
              <th className="pb-2 text-right">Share of voice</th>
              <th className="pb-2 text-right">Sentiment medio</th>
            </tr>
          </thead>
          <tbody>
            {results.map((r, i) => (
              <tr key={r.entity.id} className="border-b border-[var(--border)]/50 last:border-0">
                ...
                <td className="py-2.5 text-right">
                  {r.avgSentiment === null ? <span className="text-slate-600">in attesa</span> : (
        ...
        <p className="mt-3 text-xs text-slate-600">
          Gestisci le entità confrontate da <Link href="/settings" className="text-sky-400">Gestione progetti</Link>.
        </p>
      </section>
    </>
  );
